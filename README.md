...
Todo comando de run (`smoke`, `compare`, `screen`, `resume`, `montecarlo-scan`,
`leave-one-out`, `convergence`, `validate-top`, `null-models`) aceita
`--max-workers N` para paralelizar candidatos/pontos independentes (padrão:
número de CPUs; `--max-workers 1` força o modo sequencial original).

### Diagnósticos opcionais (fora do funil de screening)

| Quero...                                          | Comando |
|-----------------------------------------------------|---------|
| Indicador de caos MEGNO (experimental, não calibrado) | `python main.py megno --candidate <id> --budget configs/budgets/low.yaml` |
| Estatística circular formal Rayleigh/Kuiper sobre uma run (read-only) | `python main.py circular-stats --from-run runs/<run_id>` |

Nenhum dos dois entra na classificação de candidatos nem substitui o
threshold ad hoc do funil — ver `docs/LIMITACOES.md`.
