...
class ReboundEngine:
    ...
    def _extract_final_orbits(
        ...
        for offset, etno in enumerate(etnos):
            ...
            except Exception as exc:  # noqa: BLE001 - REBOUND orbit conversion can raise package-specific runtime failures; record and continue per ETNO.
    ...
    def _build_result(
        ...
        result = SingleRunResult(
            ...
            mean_angular_alignment_score=metrics["mean_angular_alignment_score"],
    ...
    def _run_rebound(
        ...
        try:
            sim.integrate(self.budget.integration_years, exact_finish_time=0)
        except Exception as exc:  # noqa: BLE001 - REBOUND may raise several integration exceptions; convert them into auditable run failures.
    ...
    def run_branch_checkpointed(
        ...
        while sim.t < target - 1e-9:
            ...
            except Exception as exc:  # noqa: BLE001 - checkpointed REBOUND runs must persist partial failure state instead of crashing the whole pipeline.
