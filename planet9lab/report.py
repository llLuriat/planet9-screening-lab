...
def build_report(
    ...
    lines.extend(
        [
            ...
            + (
                " (sem efeito físico nesta run: catálogo fixo, condições iniciais "
                "determinísticas - o seed só altera resultados para `montecarlo-scan` "
                "e para os modelos nulos)"
                if manifest.get("seed_effect", "").startswith("inert")
                else " (afeta `montecarlo-scan` e os modelos nulos)"
            ),
            "",
            "## 8. Integrador",
            "",
