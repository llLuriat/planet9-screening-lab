...
from .run import RANKING_FIELDS, metric_column_value, normalize_metric_columns, read_manifest
...
def rescore_run(run_dir: str | Path, weights_path: str | Path) -> Path:
    ...
    original_ranking = normalize_metric_columns(read_csv_dicts(run_dir / "results" / "ranking.csv"))
    rescored = []
    for row in original_ranking:
        with_metrics = {
            "apsidal_clustering_R": row["apsidal_clustering_R_with_p9"],
            "mean_angular_alignment_score": metric_column_value(row, "mean_angular_alignment_score", "with_p9"),
        ...
        without_metrics = {
            ...
            "mean_angular_alignment_score": metric_column_value(row, "mean_angular_alignment_score", "without_p9"),
