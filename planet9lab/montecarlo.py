...
# ---------------------------------------------------------------------------
# Parallel stage workers (Etapa 1). Each worker owns one stateless engine and
# returns a pure payload; the parent mutates the shared row dicts and appends
# to survivors in input order, so the funnel CSVs are byte-identical to a
# sequential run. Stage-3 checkpoints are keyed per sample_id, so workers
# never write the same file.
# ---------------------------------------------------------------------------
_STAGE_WORKER_CTX: dict = {}


def _init_stage_worker(engine: ReboundEngine, etnos: list | None, checkpoint_dir: Path | None) -> None:
    _STAGE_WORKER_CTX["engine"] = engine
    _STAGE_WORKER_CTX["etnos"] = etnos
    _STAGE_WORKER_CTX["checkpoint_dir"] = checkpoint_dir


def _stage2_worker(point: dict) -> dict:
    return run_stage2_gross_stability(point, _STAGE_WORKER_CTX["engine"])


def _stage3_worker(point: dict) -> dict:
    return run_stage3_apsidal_alignment(
        point,
        _STAGE_WORKER_CTX["etnos"],
        _STAGE_WORKER_CTX["engine"],
        _STAGE_WORKER_CTX["checkpoint_dir"],
    )
...
def run_scan(
    ...
    stage2_budget_path = config.get("stage2_budget", str(ROOT / "configs" / "budgets" / "montecarlo_stage2.yaml"))
    ...
    stage2_survivors = []
    if stage2_pool:
        stage2_outcomes = run_parallel_map(
            _stage2_worker,
            [point for point, _row in stage2_pool],
            max_workers=max_workers,
            initializer=_init_stage_worker,
            initargs=(stage2_engine, None, None),
        )
        for (point, row), outcome in zip(stage2_pool, stage2_outcomes):
    ...
    stage3_survivors = []
    if stage3_pool:
        stage3_outcomes = run_parallel_map(
            _stage3_worker,
            [point for point, _row in stage3_pool],
            max_workers=max_workers,
            initializer=_init_stage_worker,
            initargs=(secular_engine, etnos, checkpoint_dir),
        )
        for (point, row), outcome in zip(stage3_pool, stage3_outcomes):
