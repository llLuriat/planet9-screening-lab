...
# Audit V3, P2-4: the secular/apsidal model used by this codebase
# (mean_angular_alignment_score, delta_pomega_stability) was designed for the
# prograde/moderate-inclination regime typical of the Planet Nine literature
# (Batygin & Brown). It has NOT been validated for i near 90° or retrograde
# orbits (Kozai-Lidov regime). Candidates flagged by this threshold get a
# consultative blocker: the run still completes and the candidate is still
# ranked, but the limitation is recorded so it cannot be read as a validated
# result in that regime.
INCLINATION_REGIME_THRESHOLD_DEG = 80.0


def inclination_regime_blocker(i_deg: float | None) -> list[dict]:
    """Consultative (non-invalidating) blocker for candidates whose inclination
    leaves the validated prograde/moderate regime (audit V3, P2-4)."""
    if i_deg is None or i_deg < INCLINATION_REGIME_THRESHOLD_DEG:
        return []
    return [
        {
            "blocker_id": "inclination_out_of_model_regime",
            "severity": "science_limit",
            "message": (
                f"Candidato com i = {i_deg:g}° fora do regime prógrado/moderado para o qual o "
                "modelo secular/apsidal (mean_angular_alignment_score, delta_pomega_stability) foi "
                "pensado; não validado para i próximo de 90° ou retrógrado (regime Kozai-Lidov)."
            ),
        }
    ]
