...
def run_doctor() -> int:
    ...
    if latest_run.exists():
        content = latest_run.read_text(encoding="utf-8").strip()
        if not content:
            checks.append(("runs/latest_run.txt", False, "arquivo existe mas está vazio"))
            all_ok = False
        else:
            recorded = Path(content)
            if not recorded.is_absolute():
                recorded = ROOT / recorded
            if recorded.exists():
                checks.append(("runs/latest_run.txt", True, f"{content} (acessível)"))
            else:
                checks.append(
                    (
                        "runs/latest_run.txt",
                        False,
                        (
                            f"{content} (caminho gravado não existe neste ambiente - "
                            "o ponteiro veio de outra máquina ou a run foi removida)"
                        ),
                    )
                )
                all_ok = False
