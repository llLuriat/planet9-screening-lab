...
from .run import candidate_details_from_run, metric_column_value, read_manifest
...
def explain_candidate(candidate_id: str, run_dir: str | Path) -> str:
    ranking, _metrics, control = candidate_details_from_run(run_dir, candidate_id)
    manifest = read_manifest(run_dir)
    improved: list[str] = []
    worsened: list[str] = []
    for metric in [
        "apsidal_clustering_R",
        "mean_angular_alignment_score",
        "survival_rate",
        "stability_score",
        ...
        with_value = metric_column_value(ranking, metric, "with_p9") if metric != "survival_rate" else ranking.get(with_key)
        without_value = (
            metric_column_value(ranking, metric, "without_p9") if metric != "survival_rate" else ranking.get(without_key)
        )
        if with_value not in (None, "") and without_value not in (None, ""):
            try:
                delta = float(with_value) - float(without_value)
            except ValueError:
                continue
            if delta > 0:
