...
### [2026-09-26 00:54Z] MiMo (Executor) - PC: DESKTOP-DDBU1N8 - Rodada 2: Tarefa B item (a) - projecao orbital->ceu integrada ao selection-bias (impacto interpretativo declarado)

- Status: concluido; PAUSA 2 (apenas relatorio ao orquestrador; nenhuma tarefa da Rodada 3 lancada).
- **FATO (informacao do usuario 2026-09-26): a Tarefa C (4 Gyr) esta RODANDO EM OUTRA MAQUINA neste momento.** Ela NAO foi lancada nesta maquina nesta rodada (nenhuma run longa, nenhum benchmark novo) - nenhuma sessao futura deve trata-la como pendente/stalled aqui; resultados serao incorporados quando chegarem, com os numeros da maquina que rodou.
- **Protocolo de inicio:** releitura de TASK.md (Tarefa B + ultimas entradas do Log), `git log -3 --oneline` -> HEAD `b1ac738`, `git status` limpo, sync `0 0` - estado esperado batido, nenhuma divergencia.
- **Gate inicial (literal):** `.\.venv\Scripts\python.exe -m pytest -q` -> `231 passed in 268.98s (0:04:28)`; `.\.venv\Scripts\python.exe -m ruff check .` -> `All checks passed!`.
- **Estudo previo (antes de editar):** `planet9lab/geometry/sky_projection.py` (`orbital_elements_to_radec`, 9 testes, commit `b831af9`), `selection_bias.py::generate_synthetic_population` (proxy uniforme ra/dec) e `docs/LIMITACOES.md` (blocos "Atualizacao 2026-09-08" e notas "2026-09-13", incluindo a tensao epoca/catalogo em L53-63).
- **Decisao (a) - elementos orbitais sinteticos** (tudo declarado no docstring de `generate_synthetic_population`, com fonte OU pressuposto explicito): varpi ~ U(0,360) e a variavel sob teste (nulo uniforme - design angle-only de Napier et al. 2021, arXiv:2102.05601, sec. 3); omega ~ U(0,360) com Omega = (varpi - omega) mod 360 -> restricao w+Omega=varpi EXATA por objeto (ate 1 ULP de fp, teste novo); i progrado-isotropico (cos i ~ U(0,1), i em [0,90]) - **PRESSUPOSTO DE MODELO NULO declarado**, nao ajuste (os 16 objetos reais sao progrados, i ~ 10-35 graus, `data/etnos/catalog_validated.csv`); M ~ U(0,360) = fase orbital aleatoria (pressuposto de modelo nulo padrao). Nenhuma distribuicao inventada silenciosamente.
- **Decisao (b) - epoca da projecao:** JD 2456800.5 = 2014-05-23T00:00 UTC = epoca dos 13 objetos validados do catalogo real (De la Fuente Marcos & De la Fuente Marcos 2014, Tabela A1; campo `epoch` de `data/etnos/catalog_validated.csv`; LIMITACOES L53-63) -> catalogo real e populacao sintetica comparados na MESMA epoca. Constantes `SKY_PROJECTION_EPOCH_JD`/`SKY_PROJECTION_EPOCH_UTC` documentadas em `selection_bias.py` com a proveniencia e citadas no resultado (campo `sky_projection`). **Tensao declarada:** os blocos OSSOS 2013A (opt-in) antecedem a epoca 2014.4 - gravada no comentario do YAML, no caveat do resultado e no LIMITACOES; por isso `use_ossos_footprint` segue desligado (ver decisao (d)).
- **Decisao (c) - fim do proxy uniforme:** flag `generate_sky_position` REMOVIDA (a projecao nao consome draws de RNG - o motivo da flag, determinismo, era moot); todo objeto carrega `ra_deg`/`dec_deg` reais e teste novo assina a ausencia da flag. `r_au`/`delta_au` agora derivam da MESMA geometria na MESMA epoca (nova funcao `orbital_elements_to_sky` retorna ra/dec/Delta/r; `orbital_elements_to_radec` virou wrapper bit-identico - os 9 testes originais intactos); o stand-in `Delta = r + U(-1,1)` e o `nu` independente SAIRAM (coerencia geometrica testada: |Delta - r| <= r_Terra = 1,012631 UA na epoca). Nenhum caminho usa proxy residual (grep + teste de assinatura).
- **Decisao (d) - opt-in preservado:** `use_ossos_footprint` continua default `false` na config e no YAML (nao alterado; so o comentario que citava o proxy foi atualizado). Motivos registrados: footprint cobre ~0,07% da esfera (ativar por padrao colapsaria a amostra e mudaria a leitura de todo run padrao sem declarar) + tensao de epoca 2013A x 2014-05-23.
- **Decisao (e) - custo medido:** 5000 projecoes = **0,521 s (0,104 ms/objeto)** - viavel para o n_synthetic real do selection-bias-check; nenhum corte, nenhum bloqueio de custo.
- **Comparacao ANTES x DEPOIS** (run canonica `runs/screen_20260903T211311606520Z`, mesmo comando `main.py selection-bias-check --from-run ...` nos dois lados, seed 12345 historica da run, n=5000, modo uniform_filling_factor; `--help` conferido antes):

  | metrica | ANTES (proxy uniforme) | DEPOIS (projecao real) |
  |---|---|---|
  | R_real | 0.299836 | 0.299836 (inalterado) |
  | R_sint_sobrevivente | 0.032567 | 0.064584 |
  | real_exceeds_synthetic_R | True | True (valor NAO mudou) |
  | sobreviventes | 959 / 5000 | 221 / 5000 |
  | fracao de sobrevivencia | 0.1918 | 0.0442 |

  - **IMPACTO INTERPRETATIVO DECLARADO, sem suavizacao:** a amostra sintetica sobrevivente MUDOU (queda de 4,3x) e o R_sint quase dobrou - por isso o resultado do selection-bias-check muda. O booleano `real_exceeds_synthetic_R` permaneceu `True` (comportamento dos blockers da Tarefa A inalterado: favoravel nos dois lados), mas a margem mudou (0.2998 vs 0.0326 -> 0.2998 vs 0.0646) e a amostra sobrevivente e menor: com 221 objetos a dispersao nula de R fica ~1/sqrt(221) = 0,067 - leitura honesta: R_real segue bem acima de R_sint, mas a folga caiu e deve ser lida com essa resolucao. Conclusao mantida dentro do vocabulario do protocolo: triagem exploratoria; candidato de interesse dentro do protocolo (nenhum candidato encontrado / inconclusivo continuam sendo as demais saidas validas).
  - **Mecanismo verificado numericamente:** no modelo antigo as distancias usavam `nu` (verdadeira anomalia) INDEPENDENTE de M, sobrepesando objetos perto do perielio - mediana de r = 82,8 UA, P(r<=60) = 0,336; no novo, uma unica fase M uniforme gera posicao, r e Delta juntos - mediana de r = 204,7 UA, P(r<=60) = 0,059 -> menos objetos passam da profundidade. Re-sortio do stream de RNG (7 draws/objeto vs 9) e o Delta exato sao fatores secundarios.
- **Backup do artefato oficial:** antes do ANTES, o `selection_bias.json` original da run (SHA256 `9CE2230DA48BCA9F20E9ACE2A44B552B4B0EE2028B79872A475DD60BF7FF3F50`, R_sint 0.032567) foi copiado para `%TEMP%\p9_rodada2\selection_bias_OFFICIAL_PRE_RODADA2.json`; o artefato na run agora carrega os numeros DEPOIS.
- **Testes:** +10 novos (`tests/test_sky_projection_integration.py`): projecao = recalculo exato na epoca escolhida (igualdade bit-a-bit, prova contra proxy e contra outra epoca); constantes de epoca (2456800.5 / 2014-05-23); convencao w+Omega=varpi; varpi ~ nulo uniforme (R < 0,05); i progrado-isotropico; coerencia geometrica |Delta-r| <= r_Terra; posicoes sempre presentes + flag de proxy ausente; sanidade da distribuicao (mean|Dec| ~ 32,7 graus, bem abaixo dos ~45 graus do antigo proxy uniforme em dec); contrato do opt-in intacto (config + YAML); proveniencia da projecao no resultado. **2 testes existentes tiveram SO o docstring atualizado** (`test_apply_selection_function_without_footprint_keeps_uniform_behavior` e `test_selection_bias_check_default_reports_uniform_filling_mode` - o texto "proxy uniforme"/"sem posicoes de ceu" ficaria falso): assercoes intactas, nenhuma removida nem enfraquecida; nenhum teste tocado por outro motivo (os testes que pinavam comportamento, inclusive os de resultado favoravel com seed 20260903, continuam verdes sem alteracao).
- **`docs/LIMITACOES.md` atualizado:** notas 2026-09-13 (L169-176 e L217-224) agora descrevem o estado INTEGRADO; bullet do proxy (L206-216) reescrito; TODO "V = H + 5log10(r.Delta)" marcado RESOLVIDO (distancias entram desde 2026-09-07, coerentes com a projecao desde 2026-09-26 - TODO restante: funcao de fase); bloco "Proximos passos" atualizado. Limitacoes preservadas de pe (nada removido): funcao de fase ausente, cadencia real nao modelada, curva OSSOS valida so r ~21-25 / 0,50-8,00 arcsec/hora, filling factor 0,9067, "nao e probabilidade de deteccao calibrada".
- **Gate final (literal, antes dos commits):** `.\.venv\Scripts\python.exe -m pytest -q` -> `241 passed in 273.75s (0:04:33)` (231 baseline + 10 novos); `.\.venv\Scripts\python.exe -m ruff check .` -> `All checks passed!`. Re-rodagem do gate no estado pos-commit e reportada no relatorio da PAUSA 2.
- **Commits:** (1) `4fd0a0c` "feat(selection-bias): projecao orbital->ceu integrada ao pipeline (Tarefa B item a)" - mensagem declara o impacto interpretativo com os numeros antes x depois; (2) esta entrada (hash no relatorio da PAUSA 2). Push para `https://github.com/llLuriat/planet9-screening-lab.git` + sync `0 0` verificados no fechamento.
- Arquivos: `planet9lab/selection_bias.py`, `planet9lab/geometry/sky_projection.py`, `configs/science/observational_bias.yaml` (somente comentario), `docs/LIMITACOES.md`, `tests/test_selection_bias.py` (2 docstrings), `tests/test_sky_projection_integration.py` (novo), `TASK.md` (esta entrada).
- Proximo passo: PAUSA 2 - apenas o relatorio; robustez/stale links so com instrucao seguinte. Tarefa C: rodando em outra maquina (acima).
- PC: DESKTOP-DDBU1N8
### [2026-09-26 01:26Z] MiMo (Executor) - PC: DESKTOP-DDBU1N8 - Rodada 3 (final): backup pré-projeção versionado, robustez executada (LOO + modelos nulos) e stale links do Artigo_FEBRACE corrigidos

- **Protocolo de início:** HEAD `e8a7ca9`, árvore limpa, sync `0 0`; gate inicial `241 passed in 274.77s (0:04:34)` + `All checks passed!`.
- **Item A — backup pré-integração versionado (rastreabilidade):** `git check-ignore -v` apontou `.gitignore:37: runs/*/` (só `runs/README.md` é rastreado; `git ls-files runs/screen_20260903T211311606520Z --error-unmatch` falhou = run **ignorada**, não rastreada) → destino `docs/hermes/artefatos/selection_bias_PRE_SKY_PROJECTION_20260925.json`. SHA256 do backup do %TEMP%: `9CE2230DA48BCA9F20E9ACE2A44B552B4B0EE2028B79872A475DD60BF7FF3F50` (R_sint 0,032567; sobreviventes 959/5000 = 0,1918; R_real 0,299836; real_exceeds_synthetic_R True — os números ANTES citados no artigo v2, agora auditáveis a partir do repo). A cópia versionada adiciona apenas a chave `_provenance` (SHA256 do original, origem no %TEMP%, contexto antes×depois); o round-trip `json.dumps(indent=2)` + CRLF == bytes originais foi verificado antes de gravar, então o conteúdo científico é byte-idêntico.
- **Item B1 — custo estimado ANTES de executar:** budget da run (manifest medium): `integration_years: 200.0`, `null_model_integration_years: null` (modelos nulos usam os mesmos 200 anos, sem sub-orçamento). Âncora empírica: o screen completo da run levou 0,672 s (8 pares de controle, 16 ETNOs, paralelo). Estimativas registradas: LOO `--top 5` = 5 × 16 = 80 tarefas ≈ 7–15 s de parede; null-models = 3 modelos × 20 shuffles × top 5 ≈ 130 pares ≈ 11–20 s; sensibilidade com folga 10× < ~5 min → total muito abaixo do teto de ~1 h, **nada ficou pendente de autorização**.
- **Item B2 — executado com flags literais:**
  - `.\.venv\Scripts\python.exe main.py leave-one-out --from-run runs/screen_20260903T211311606520Z --top 5` → **2,0 s**; 20 linhas = 5 candidatos × 4 ETNOs (o `_context` usa o catálogo default `catalog_v2` + `etno_selection.yaml`, que seleciona 4 das 5 linhas — não os 16 da run; comportamento pré-existente, a interface não tem flag `--etnos`). `robustness_score` = **1,0** para os 5 candidatos (4/4 remoções com delta > 0): `p9_row8_bb21_bestfit` delta 0,204331 ± 0,00756; `p9_row1/row4/row5/row6` ≈ 0,0860 ± 0,0147.
  - `.\.venv\Scripts\python.exe main.py null-models --from-run runs/screen_20260903T211311606520Z --top 5 --models shuffle_varpi,randomize_angles,no_p9_catalog_baseline` → **2,1 s**; 300 linhas; `n_shuffles=20` (default — mínimo para percentil 95); `max_workers` default (8).
- **Item B3 — resultado real (mesmo peso se favorável ou desfavorável):** `shuffle_varpi` passou em 4/5 (percentil 100, p_like 0,047619; falhou apenas `p9_row8_bb21_bestfit`, percentil 0, p_like 1,0); `randomize_angles` passou em 0/5 (percentis 20–75, p_like 0,285714–0,809524); `no_p9_catalog_baseline` passou em 0/5 (percentil 0, p_like 1,0). **Nenhum dos 5 candidatos supera os 3 modelos nulos ao mesmo tempo → blocker `null_model_not_exceeded` adicionado à run** (blockers.json agora com 2: `etno_catalog_not_fully_validated` + `null_model_not_exceeded`). Leitura dentro do protocolo: triagem exploratória em que nenhum candidato dos 5 se sustenta sobre os três modelos nulos — resultado desfavorável, coerente com o histórico do projeto, reportado com o mesmo peso. O que os comandos atualizaram (nada fora da run, que é gitignored): `robustness/leave_one_out.{csv,summary.json}`, `robustness/null_models.{csv,summary.json}`, 4 novos eventos em `events.log`, `report.md` regenerado (`leave_one_out_status: run`, `null_models_status: run`). `candidates_results_cache.json` **não** foi editado à mão (segue `leave_one_out_status: not_run`, `null_models_status: not_run`, `robustness_score: not_computed` — artefato da run).
- **Item B4 — LIMITACOES atualizado (4 trechos, sem apagar limitações de pé):** LOO e modelos nulos agora com estado real (executado em 2026-09-26 nesta máquina, na run canônica, com os números acima); bloco "Resultado científico honesto" ganhou a Atualização 2026-09-26; item do `p9_row8`: LOO/nulos executados, propagação de incerteza, convergência e detectabilidade seguem `not_run`. Preservados: fase de detecção, cadência/curva OSSOS, filling factor, "não calibrado como probabilidade de detecção".
- **Item C — stale links corrigidos (8/8):** `configs/experiments/angle_robustness.yaml:7` (texto de documentação em `description: >-`, não valor funcional; a string era só o nome do arquivo, sem prefixo `docs/` → virou `docs/archive/Artigo_FEBRACE_revisado.docx`, revalidado com `yaml.safe_load`), `docs/LIMITACOES.md:464` (era 430), `docs/AUDITORIA_V3_PLANET9_SCREENING_LAB.md:21`, `docs/CONSOLIDADO_PLANET9_SCREENING_LAB.md:66/284/292/331`, `docs/CANDIDATOS_QUADRO2.md:111`. Não tocados por regra: `TASK.md` (Log append-only) e `docs/historico/`. Verificação pós-edição: restam matches de `docs/Artigo_FEBRACE_revisado.docx` apenas em `TASK.md:797` (entrada histórica) e `docs/historico/` — zero em arquivos vivos.
- **Gate final:** `241 passed in 275.07s (0:04:35)` + `All checks passed!` (241 = baseline da Rodada 2; nenhum teste novo nesta rodada).
- **Commits:** `5cbbf4e` (backup + 8 stale links) e este (robustez + LIMITACOES + Log); push com sync `0 0`, árvore limpa.
