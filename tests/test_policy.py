from planet9lab.policy import (
    INCLINATION_REGIME_THRESHOLD_DEG,
    classify_candidate,
...
def test_inclination_prograde_no_blocker():
    assert inclination_regime_blocker(20.0) == []


def test_inclination_at_threshold_blocker_consultative():
    blocker = inclination_regime_blocker(INCLINATION_REGIME_THRESHOLD_DEG)
    assert len(blocker) == 1
    assert blocker[0]["blocker_id"] == "inclination_out_of_model_regime"
    assert blocker[0]["severity"] == "science_limit"


def test_inclination_retrograde_blocker():
    assert inclination_regime_blocker(95.0)[0]["blocker_id"] == "inclination_out_of_model_regime"


def test_inclination_none_no_blocker():
    assert inclination_regime_blocker(None) == []
